...
>
> **Last verified:** `342fc8d` · 2026-05-28
>
...
- **Ingestion check-in false-positive post-filter.** `/api/courses/[code]/checkin` post-filters the LLM's `highlights[]` to drop "missing syllabus" when the Sheets catalog already has both objectives + projects (catalog effectively replaces the syllabus content), and drops "missing rubrics for major projects" when Canvas Assignments is in the materials list (per-assignment rubrics live inside its digest). Other LLM signals (FERPA, stacked auto-set-asides, near-empty digests) still surface.
