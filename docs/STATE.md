...
**Stage 3 (Audit-chat agent loop) shipped 2026-05-27**: `capture-chat-agent` function with three retrieval tools (`list_materials`, `fetch_material_section`, `search_materials`) against per-course Weaviate tenants; structured per-turn response (finding + question + citations + readiness); CaptureChatPanel UI with citation chips + session continuity; live-smoke fixes (campus.ts manual two-phase tool-call loop; audit-tools graceful empty-tenant handling). Stages 4–5 (synthesis rewrite, legacy migration) remain.
...
| --------- | ---- | ----------- |
| **CourseCapture v2 — Agentic Retrieval** | [spec](./superpowers/specs/2026-05-26-coursecapture-agentic-retrieval-design.md) | Three-phase architecture: per-material ingestion (chunk + digest + Weaviate index), tool-using audit agent, synthesis with intrinsic provenance. **Stage 1 (foundation) + Stage 2a (pre-Weaviate ingestion: chunker, FERPA, policy, digest, contextualizer, vector-store abstraction with in-memory backend, wired into finalizeExtraction behind `COURSECAPTURE_V2_INGESTION=1`) shipped 2026-05-26. Stage 2b (WeaviateVectorStore against local Weaviate; Materials panel UI: indexing-status dots, FERPA pills, auto-set-aside override, audit-mode toggle; ingestion-checkin AI function + inline panel above audit chat; admin v2-backfill route; chunker UUID-id fix) shipped 2026-05-27. Stage 3 (audit-chat agent loop: `capture-chat-agent` function, three retrieval tools — `list_materials` / `fetch_material_section` / `search_materials` — against per-course Weaviate tenants, structured per-turn response with finding + question + citations + readiness, CaptureChatPanel UI with citation chips + session continuity; live-smoke fixes: campus.ts manual two-phase tool-call loop, audit-tools graceful empty-tenant handling) shipped 2026-05-27.** Stages 4–5 (synthesis rewrite, legacy migration) still ahead. |
