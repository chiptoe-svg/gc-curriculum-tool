...
| **Microphone bridge via Tailscale Funnel** (2026-06-03) | Browsers require HTTPS (or localhost) for `getUserMedia()`, but the main app stays plain HTTP at the Clemson LAN IP — to avoid cert warnings + per-device installs. Solution: a hidden HTTPS iframe at `<machine>.<tailnet>.ts.net/voice-bridge` (Tailscale Funnel) records the audio and posts the transcript back to the LAN-HTTP parent via `postMessage`. Only three paths exposed over the Funnel (`/voice-bridge`, `/api/transcribe`, `/api/voice-session`); rest of app stays LAN-only. Auth: 24-hour session token issued by the LAN-HTTP main app (so attackers without LAN access can't get one), bound to slug+IP, validated on every `/api/transcribe` call along with Origin pinning + per-slug rate limit (30/hr). Iframe lazy-mounts on first mic click and persists for the page session — turns 2-30 in an audit have zero added round-trips. Files: `app/voice-bridge/`, `app/api/voice-session/`, `components/VoiceBridgeProxy.tsx`, `lib/voice-session/store.ts`, `lib/rate-limit/slug-rate-limit.ts`, `scripts/setup-tailscale-funnel.sh`. Plan: [`2026-06-03-microphone-bridge-via-tailscale.md`](./superpowers/plans/2026-06-03-microphone-bridge-via-tailscale.md). | live | 2026-06-03 |
