...
| --------- | ---- | ----------- |
| **CourseCapture v2 — Agentic Retrieval** | [spec](./superpowers/specs/2026-05-26-coursecapture-agentic-retrieval-design.md) | Three-phase architecture: per-material ingestion (chunk + digest + Weaviate index), tool-using audit agent, synthesis with intrinsic provenance. **Stage 1 (foundation) + Stage 2a (pre-Weaviate ingestion: chunker, FERPA, policy, digest, contextualizer, vector-store abstraction with in-memory backend, wired into finalizeExtraction behind `COURSECAPTURE_V2_INGESTION=1`) shipped 2026-05-26. Stage 2b (WeaviateVectorStore against local Weaviate; Materials panel UI: indexing-status dots, FERPA pills, auto-set-aside override, audit-mode toggle; ingestion-checkin AI function + inline panel above audit chat; admin v2-backfill route; chunker UUID-id fix) shipped 2026-05-27.** Stages 3–5 (agent loop, synthesis, migration) still ahead; order: Stage 3 (agent loop), Stage 4 (synthesis rewrite), Stage 5 (legacy migration). |
