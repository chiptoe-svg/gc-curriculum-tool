...
>
> **Last verified:** `5efb547` · 2026-06-04
>
...
- **DB off Neon, always-on hosting.** Deferred to deployment-planning phase. **Backup/restore is no longer deferred** — four-tier scheme shipped 2026-06-01: (1) Neon's own ~7h PITR (continuous, free tier); (2) local `pg_dump`/gzip every 6h to `~/Library/Application Support/gc-curriculum-tool/backups/` via `scripts/backup/pg-snapshot.sh` + `com.gc.pg-backup` launchd plist, auto-prunes >365d (Library path chosen over `~/Documents/` because launchd-spawned processes can't enumerate `~/Documents` without TCC Full Disk Access); (3) monthly Neon branch checkpoint on the 1st of each month (`monthly-YYYY-MM`, rolling window of 9 — leaves headroom under free-tier 10-branch cap) via Neon REST API when `NEON_API_KEY` + `NEON_PROJECT_ID` are set; (4) weekly off-site push to private repo `chiptoe-svg/gc-curriculum-backups` on Sundays, retained forever. Restore procedure in that repo's README.
