...
>
> **Last verified:** `b1bba70` · 2026-05-28
>
...
| --------- | ---- | ----------- |
| **CourseCapture v2 — Agentic Retrieval** | [spec](./superpowers/specs/2026-05-26-coursecapture-agentic-retrieval-design.md) | Three-phase architecture: per-material ingestion (chunk + digest + Weaviate index), tool-using audit agent, synthesis with intrinsic provenance. **Stages 1, 2a, 2b, 3, 4 shipped 2026-05-26/27 (see Active arc for the per-stage breakdown). Stage 6 (agent persona + discipline lift) shipped 2026-05-28: Manning-skill discipline anchor in `capture-chat-agent.md`; explicit persona + Evidence Rule + disagreement triangulation; full 5/5 disposition probe bank; periodic synthesis + self-correction + pre-wrap-up turn discipline; `ToolDefinition.usagePolicy` field + `renderToolDescription` helper with per-tool policies co-located in `audit-tools.ts`.** Stage 5 (legacy migration: `captureConversations` → `capture_messages`, "Legacy draft" banner, citation drawer) and Stage 7 (session-continuity briefing, faculty profiles, streaming) are the remaining v2 tracks. |
