...
>
> **Last verified:** `3db19c4` · 2026-05-27
>
...
| --------- | ---- | ----------- |
| **CourseCapture v2 — Agentic Retrieval** | [spec](./superpowers/specs/2026-05-26-coursecapture-agentic-retrieval-design.md) | Three-phase architecture: per-material ingestion (chunk + digest + Weaviate index), tool-using audit agent, synthesis with intrinsic provenance. **Stages 1, 2a, 2b, 3 shipped 2026-05-26/27 (see Active arc for the per-stage breakdown). Stage 4 (synthesis with source provenance) shipped 2026-05-27: `capture-synthesis.md` prompt, mechanical source-flag derivation (`lib/ai/synthesis/source-derivation.ts`), optional `source` + `citations` on every CaptureProfile finding, scores route branches v2 transcript from `capture_messages`, ProfileReviewPanel + VerificationSummary render source-indicator badges.** Stage 5 (legacy migration: `captureConversations` → `capture_messages`, "Legacy draft" banner, citation drawer) is the only remaining v2 task. |
