...
| 2026-06-04 | [`plans/2026-06-04-careercapture-v1.md`](./plans/2026-06-04-careercapture-v1.md) | ✅ Done. Per-career-target AI-conducted employer interview at `/partners/[token]/interview/[targetId]`. New schema (`career_capture_messages` + `career_captures`, migration 0028), two new default-tier AI functions (`capture-employer-chat-agent` + `capture-employer-synthesis`), partner-side voice via `/api/partners/transcribe`. Synthesis emits a structured `CareerCaptureProfile` (role shape + day-1 K/U/D + dealbreakers + hiring signals + divergence from catalog). 12 tasks via subagent-driven execution. |
