...
| 2026-06-05 | [`plans/2026-06-05-prerequisite-edges.md`](./plans/2026-06-05-prerequisite-edges.md) | ✅ Done (shipped 2026-06-05). Persisted skill-tagged prerequisite edges: `prerequisite_edges` table (migration `0030_supreme_iron_monger.sql`) + `prereq-edge-seed` AI function (count 19→20) + deterministic `computePrereqGaps` engine (ordinal-MAX, no double-count, `basis: measured|intended|none`) + admin routes (`/api/admin/courses/roster`, `/api/admin/courses/[code]/prereq-edges`) + `/courses` roster surface (data-state badges + bulk preload + add-a-class) + `/courses/[code]` prerequisite confirm/edit + gap view. Subsumes Phase 1C. Deferred: `intended`-basis increment + program-wide graph viz. |
| 2026-06-05 | [`plans/2026-06-05-intended-skills-rough-pass.md`](./plans/2026-06-05-intended-skills-rough-pass.md) | ✅ Done (shipped 2026-06-05). Intended-skills rough pass: `course_intended_coverage` table (migration `0031_clumsy_selene.sql`) + `intended-skills-extract` AI function (count 20→21) + gap-engine `basis:'intended'` seam + `POST /api/admin/courses/intended-skills` admin route (`mode:'one'`/`'all-uncaptured'`, daily-cap gated). Deferred: in-capture reference-panel. |
