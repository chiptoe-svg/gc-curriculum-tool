...
- [~] **A6. Run the reliability study.** ✅ **Parts (i)+(ii) 2026-06-12** — synthesis 5× across GC 3800, GC 4060, GC 3460 + coverage-scorer 5× across GC 3800 × {Account Management, Brand Strategy}. Cost ~$2.20. **Headlines:** synthesis is extremely stable (across-run SD ≈ 0.02–0.05 per dimension; foundational D band-agreement 100%); coverage scorer is LOW reliability (mean D band-agreement 25% across 2 pairs; Account Management 0%, Brand Strategy 50% — with within-±1 integer agreement at 100% U, 83.3% K for Brand Strategy suggesting scores cluster near a band boundary). **The coverage-scorer model was `gpt-5.4-mini` vs. `gpt-5.4` for synthesis** — mini-model variance drives the warning. Full data: `docs/superpowers/audits/2026-06-12-reliability-study.json`, report `docs/superpowers/audits/2026-06-12-reliability-study.md`. Script: `scripts/_one-off/reliability-study.ts` (read-only, re-runnable). **(iii) Faculty raters still pending** — 2–3 faculty independently hand-scoring one course against `depth-scale.md` → Krippendorff's α ≥ 0.70. Parts (i)+(ii) done; part (iii) open.
