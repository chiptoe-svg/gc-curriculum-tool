...
- [~] **A6. Run the reliability study.** ✅ **Parts (i)+(ii) 2026-06-12** — synthesis 5× across GC 3800, GC 4060, GC 3460 + coverage-scorer 5× across GC 3800 × {Account Management, Brand Strategy}. Cost ~$2.20. **Headlines:** synthesis is extremely stable (across-run SD ≈ 0.02–0.05 per dimension; foundational D band-agreement 100%); coverage scorer LOW on mini: mean D band-agree 25% across 2 pairs. **The coverage-scorer model was `gpt-5.4-mini`** — mini-model variance drives the warning. ✅ **Part (ii) A/B heavy re-run 2026-06-13** — same 2 pairs re-run on gpt-5.5 (heavy) after promotion; cost $0.82. **A/B headline: D band-agree 25.0% (mini) → 73.3% (heavy) +48.3pp; U 41.7% → 100.0% +58.4pp; K 25.0% → 81.7% +56.7pp. Heavy stability is materially better; re-score of 120 pre-promotion mini cells RECOMMENDED (~$3.60 est.).** Full data: `docs/superpowers/audits/2026-06-12-reliability-study.json`, report `docs/superpowers/audits/2026-06-12-reliability-study.md`. Scripts: `reliability-study.ts` (original) + `reliability-study-part2b.ts` (A/B). **(iii) Faculty raters still pending** — 2–3 faculty independently hand-scoring one course against `depth-scale.md` → Krippendorff's α ≥ 0.70. Parts (i)+(ii)+A/B done; part (iii) open. ✅ **Tripwire + UI surfacing shipped 2026-06-12** — standing `pnpm reliability:check` (scripts/reliability/check.ts) re-runs on the same fixed pairs, exits non-zero below RELIABILITY_THRESHOLDS; pre-commit nags on scoring-surface changes; /program cell drawer shows last-measured stability per model. Closes the blind spot that let the mini drift go unnoticed.
