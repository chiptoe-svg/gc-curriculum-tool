# Tiered Ingestion — Increment 2b: Triage UI + Ingest Action

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development. Checkbox steps.

**Goal:** The first user-visible piece of tiered ingestion: when `COURSECAPTURE_TRIAGE=1` and a Canvas import returns a manifest, show a **triage screen** grouping materials by tier (high/middle/background) with move up/down, ignore, delete, a lecture-slides nudge, and a skipped list — then an **Ingest & continue** button that enqueues the kept rows and advances to the interview. Part of the [tiered-ingestion-triage spec](../specs/2026-06-18-tiered-ingestion-triage-design.md); builds on 2a (tiers persisted on `course_materials`).

**Architecture:** Reuses existing endpoints — `PATCH /materials/[id]` (ignore, +new `tier`), `DELETE /materials/[id]` (delete), `POST /admin/v2-backfill` (Ingest = enqueue all non-ignored). The triage screen is a new `landingStep === 'triage'` rendered inside CaptureClient's `'chat'` stage, between the materials step and the interview. Time estimates are deferred to Increment 4 (stub/omit here). **Note:** until Increment 3, the worker ignores `tier` — Ingest runs the existing full pipeline for every kept row; the triage UI still records tiers for when Inc 3 lands.
