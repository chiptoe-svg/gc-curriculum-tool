...
3. **Skip low-content slides.** `content_level: "low"` (title/agenda/divider/thank-you) → kept as a one-line note, **not embedded**. This bounds the big-deck cost; the skip signal is free (same call).
4. **Two-level output:** per-slide notes (embedded individually) + a short **deck rollup** stitched from the notes ("what this unit covers").
5. **Vision-primary.** Text-dense/code/data-table slides are the one weak spot for VLM OCR; **default vision-only**, with a *noted future refinement* to supplement flagged dense slides with Docling text. Not in v1 unless walkthrough shows it's needed.

### Retrieval vs citation granularity (decoupled — resolves the "creepy" concern)
- **Embed per-slide** for retrieval quality. Marginal cost is just embedding calls (campus Qwen, $0) on top of notes we generate anyway — nearly free.
- **Cite at the deck level.** The slide *index* is internal metadata only; it is **never surfaced**. The agent/synthesis attributes to "the Lecture 9 deck," and the citation drawer shows the retrieved note headed by the deck name — never "slide 4." (The number is what felt invasive, not the content.) This is a labeling + prompt decision, independent of the embedding choice.
...
- Rough math: background ≈ one digest (~1 s); middle ≈ render + (content-slides × ~1 s); high-value file ≈ Docling (~s/page) + (chunks × ~0.5 s contextualize) + embed.
...
- **New:** (a) Phase-1 list mode (import without extraction/enqueue) + cheap size-probe; (b) the triage page + classifier; (c) a per-material **`tier`/depth** column the worker branches on; (d) the **per-slide vision** path (render → VLM note → per-slide embed + deck rollup); (e) the estimate model.

## What this does NOT include
- No change to the interview (Step 2) or synthesis logic beyond the deck-level citation labeling.
...
## Testing
- **Phase-1 list:** import yields a manifest with size signals and *no* extraction/enqueue side effects.
...
- **Middle tier:** render produces N images; vision note schema validates; `content_level: low` slides are skipped from embedding; per-slide notes embed; citation surface contains the deck name and **never** a slide index (assert the index is absent from any user-facing field).
- **Worker tiering:** `processMaterial` honors the `tier` column (full / vision / digest paths).
