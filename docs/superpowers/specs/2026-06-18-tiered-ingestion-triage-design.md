...
**Encourage lecture slides.** Lecture decks are among the strongest instructional evidence (what was actually taught/explained) and the middle tier is built to ingest them well — but they're frequently *not* in Canvas (faculty present from local files or Google Slides), so discovery alone misses them. Phase 1 surfaces a **positive, add-a-deck nudge**: prominent when **no deck/slide-type material is detected** in the manifest ("We didn't find any lecture slides — they're some of the best evidence of what you taught. Add them?"), and a lighter "add more decks" affordance when some are present. Framing stays invitational ("more is better"), never a deficiency flag (consistent with the Round-10 problem-solving de-nag). The nudge offers a direct upload/add path; added decks default to the **middle** tier.
