...
Three hypotheses, one resolved conceptual question, one actionable first step — all of them applications of one keystone.

- **The keystone (why A, B, and C are one system, not three):** **KUD+depth is the unit of account the whole framework is denominated in** — a common Know/Understand/Do × depth axis on which a course's *output*, a downstream course's *prerequisite expectation*, the program's *aggregate coverage*, and a career's *measured demand* are all expressed. Shared denomination → boundary-crossing questions become **arithmetic on a shared axis**, not translation between mismatched vocabularies (prereq-gap = a subtraction; curriculum = a roll-up; career-readiness = a same-axis coverage-vs-demand comparison). It is a **sound internal currency** (course→curriculum, one authored taxonomy) and a **pegged-but-lossy external one** (into careers — hence A's match-quality stamps). A bounds its external convertibility, B prices its decay, C governs its arithmetic and its truth-claims. Full framing: §"The keystone" below.
...
## The keystone — KUD+depth as the unit of account (the universal currency)

A, B, and C are not three separate ideas; they are three uses of one underlying move. **KUD+depth is the unit of account the whole framework is denominated in** — the common currency that lets value move *between classes, through the curriculum, and into career paths* without being re-translated at each boundary. This section names the move explicitly, because everything else in the memo is an application of it.

**A currency needs three properties, and KUD+depth has all three:**
- **A common denomination.** Every entity is expressed on the same axes — competency × {K, U, D} × depth 0–5: a course's *output*, a downstream course's *prerequisite expectation*, the program's *aggregate coverage*, and a career's *measured demand*. Same units → commensurable at all.
- **A medium of exchange.** Because the units match, boundary-crossing questions become *arithmetic on a shared axis* instead of translation:
  - **class ↔ class (Q2):** prerequisite gap = expected-entry depth − supplied depth — a subtraction, not a fuzzy title match;
  - **through the curriculum (B):** a competency's K/U/D trajectory is *traced and rolled up* across the sequence — the thread, its accumulation, its decay, its reinforcement;
  - **into career (A):** a destination decomposes into the *same* sub-competency × K/U/D structure, so readiness is coverage-vs-demand on one axis — a *same-axis comparison rather than a translation*.
- **An evidence-anchored store of value.** The evidence-above-zero rule is the hard-money discipline; it is what stops the currency inflating on aspirational syllabus verbs (a K-score bought with recall-verbs alone is counterfeit).

**Why no other unit works — the alternatives all fail at the boundary-crossing job.** Course *names/titles* don't translate (a "Color" course vs. a role's "color management" need); *text / embeddings* are a recall-stage aid only — lossy, subject to the §5.1 leaderboard trap and A's name-mismatch; *O\*NET's 35 cross-occupational skills* are the context-free transferable-skill construct the framework rejects (`background.html` §11); *Bloom* has no Do axis and is the wrong grain for cross-program mapping; *credit hours / grades* measure seat-time and relative ranking, not attained capability. KUD+depth is the only unit simultaneously fine-grained enough to be honest, universal enough to be shared across all three levels, and pegged to student-side evidence rather than aspiration.

**The currency-soundness caveats — they are exactly A, B, and C.** Naming KUD as the currency does not make it frictionless; it locates *where* the friction is, and each friction is one of the hypotheses:
- **FX spread at the career boundary (A).** The currency converts *cleanly within* the curriculum (one authored taxonomy) but *imperfectly into* careers — the entire reason A is many-to-many, partial, never 1:1, match-quality-stamped, with "no clean match" as a first-class finding. Internal currency: clean. External: pegged-but-imperfect.
- **A carry / decay cost (B).** Instantaneous depth ≠ durable depth, so cross-level arithmetic must spend *durable* depth, not the instantaneous max (A "maps durable depth"). The currency framing sharpens B rather than competing with it: bleed-off is the depreciation of held KUD.
- **Interval-scale arithmetic + uncertainty (C / §4.2).** Rolling up ordinal KUD treats it as interval, and its sums travel with an SEM band, not as points — the currency is real, its arithmetic is honest only when it carries the interval.
- **Commensurable, not true (C).** A shared currency buys *contestable, aggregable comparison* — the comparative claim ("more evidence-grounded than committee oral tradition"), never the absolute one.

**Net.** The framework's core bet in one line: *a single evidence-anchored K/U/D × depth currency can span course → curriculum → career, where every alternative unit breaks at the boundaries* — and A, B, C are the disciplines that keep that currency sound (A bounds its external convertibility, B prices its decay, C governs its arithmetic and its truth-claims). This is also why the embeddings question resolves the way §5 says it does: KUD+depth is not a representation *competing* with embeddings — it is the *denomination the comparisons are settled in*.
