...
- **AI drafts from a HELD-OUT set** — score N of the ~10 currently-unscored captures — and the **original 6-snapshot fixture is reserved for validation only**. Author-set and test-set share no snapshot. **Label the authoring set with the CROSS-FAMILY model (glm-5.2), NOT gpt-5.6-sol** — because sol is a judge in the validation gate (R3), and letting the same model both shape the negatives the matcher learns from and grade the result reintroduces a judge-reuse circularity (sol self-judge optimism was measured at 3–11 pts).
