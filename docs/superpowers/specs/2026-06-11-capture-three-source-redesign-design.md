...
## Resolved in review — round 2 (operator, 2026-06-11)

5. **Course-overview summary at the top, unrollable (Box 1 look).** The operator likes the existing "Catalog (from the course sheet)" block (DESCRIPTION · PREREQUISITES · LEARNING OBJECTIVES · MAJOR PROJECTS · REQUIRED INCOMING SKILLS). That block currently lives *inside* `MaterialsPanel`. **Extract it into a reusable presentational `CatalogOverview` component** and use it as Box 1's **unrolled** content; Box 1 sits at the **top of the window** as a course-overview summary (collapsed → a one-line summary of the course; unroll → the full `CatalogOverview`). Source label: "from the Google Sheet / course catalog / syllabus." Keep `MaterialsPanel` using the same extracted component (DRY).
6. **`MaterialsPanel`'s home = "the materials manager."** Per "stick it under the materials manager": the kept deep panel lives behind a **Materials manager** entry (its own surface/route, e.g. from Course Builder), not on the Step-1 confirm screen. The three boxes are the Step-1 surface; the manager is the power-user deep dive.
