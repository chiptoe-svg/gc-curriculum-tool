...
- **Operator visibility = a list + full results.** Two parts: **(a) discovery** — `/admin` (faculty-gated, alongside the mint panel) lists the **created sandbox courses** (display title, capture status) each with a **Review in capture** link (`/capture/<EXT-…>?slug=` — the app's established faculty-nav pattern, on the Basic-Auth-gated capture surface) and a **Download bundle** button. (They also appear in the faculty `/courses` "External / sandbox" section.) **(b) full results** — `isCourseReadableBy` gains an **operator override read from the `Authorization: Bearer` header only** (ADMIN_TOKEN or the slug — `operatorBearerOk`): `isProgramVisible(course) || operatorBearerOk(req) || boundSession`. **Never a URL query** — these are public routes, so a query secret would leak via access logs + the Referer header (this was caught + fixed in security review; an earlier `?slug=` query variant was reverted). The "Download bundle" button does a **header-only Bearer fetch** of `/okf-bundle` and triggers a client download, so the operator gets the full result with no secret in any URL. The tester's own access stays via their session; no slug ever reaches them.
