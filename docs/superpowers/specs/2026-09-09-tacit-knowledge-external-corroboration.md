...
**Does not:**
- It is **not measurement**. The authors call their codified/tacit indices "a simple descriptive exercise."

  > **⚠️ CORRECTION, same day (2026-09-09), to §2 and §3 of this memo and to advice I gave the GC Alumni session.** I described the indices as built from O*NET inputs — "codified ≈ required formal education + O*NET taught-content domains; tacit ≈ required work experience + on-the-job training + experiential domains" — and told that session the inputs were therefore public and reproducible from their own `onet_descriptor` table. **That is the paper's BODY TEXT, not its method.** Their Appendix J (read by the alumni session, which I had not read) shows the actual construction: **GPT-5.4-mini at temperature 0 scoring 7,514 ADP job titles *and descriptions* on two 1–10 scales**, averaged to SOC and split at an employment-weighted mean. The O*NET elements are *worked examples in the model's system prompt* that teach it the distinction — they are not the index. **The index is therefore not reproducible outside ADP**, and it is a **third LLM-derived layer** in this paper, alongside the two exposure measures (§4). What *is* reusable is their published **taxonomy** — Appendix J prints the full assignment of O*NET knowledge domains, work activities and cross-functional skills to codified vs tacit; the alumni session verified all 48 elements against their `onet_descriptor` by exact name.
...
**Two findings from the alumni session's attempt (2026-09-09) that bound this further.**

*(i) The codified gradient is largely education in disguise.* The paper's own footnote reports that the **codified** gradient is **not significant once college share is controlled**, "demonstrating its overlap with education", while the **tacit** gradient for experienced workers survives. So the durable half of the mechanism is the tacit half — which is the half this framework's Do axis addresses, and the half that cannot be delivered by a syllabus alone.

*(ii) The mechanism resists measurement at occupation granularity — a negative result worth more than a positive one would have been.* They applied the published taxonomy to O*NET importance × level per SOC (same definition, different instrument) and found it **does not discriminate on an occupationally concentrated population**: their alumni cluster in a few SOCs sitting near the middle of the tacit distribution (Graphic Designers alone is 104 of 173 scored jobs in the high-exposure/shrinking cell, at tacit z = +0.10). **61% of that cell lies within ±0.10 z of the threshold; a quarter-standard-deviation change in an arbitrary cut inverts the conclusion** (z = 0.00 → 126 high-tacit/47 low; z = +0.25 → 22/151). Shipping the z = 0.00 answer would have manufactured "126 of our at-risk jobs are concentrating into higher-expertise work" purely from where the line was drawn. **This strengthens the framework's position rather than weakening it:** if the mechanism cannot be measured at occupation granularity on a concentrated population, that is a further reason the destination construct cannot be a SOC-keyed score — which is exactly what §11 already argues.
