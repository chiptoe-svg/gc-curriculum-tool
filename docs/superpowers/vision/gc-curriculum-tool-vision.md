...
**Why not Bloom's Taxonomy?** Bloom's six-level cognitive taxonomy (Bloom, 1956; Anderson & Krathwohl, 2001) is the right tool for analyzing the cognitive demand of an *assessment task* — it discriminates finely between types of thinking and is well-suited to writing and evaluating assessments. That is a different job than the one this tool has to do. The deeper reason for KUD is fit to purpose, not merely compactness: **Know / Understand / Do is the native vocabulary of career competence** — the destination the tool maps toward. Occupational models describe a role in terms of required knowledge, the work activities and tasks a person must perform, and the judgment to carry both into unfamiliar cases — which is Know, Do, and Understand almost one-to-one. Mapping curriculum coverage onto a career is then a *same-axis comparison rather than a translation*, whereas Bloom's "Analyze vs. Evaluate vs. Create" maps cleanly onto neither what a course builds nor what a job requires. **The Do axis is decisive for a second reason Bloom lacks entirely:** Bloom is purely cognitive, with no performance dimension — yet a career is defined largely by what its holder can *do*, and this program treats problem-solving as a habit built by doing under difficulty over time (depth × productive failure × reflection), legible only on Do/Understand evidence that accumulates across courses, which Bloom has no place to record. Compactness is then a further operational dividend: three categories with internal depth keep faculty review tractable across hundreds of (course × competency × dimension) cells, where Bloom's finer distinctions — Analyze vs. Evaluate, Apply vs. Create — would compound into boundary disputes that make review harder, not easier. Bloom remains the right lens for assessment design and continues to inform the audit-conversation probes used inside each cell.
...
- **A flag/dispute trail.** [LIVE] Every AI judgment on the program matrix and every competency in a review profile can be flagged with a named faculty note. Flags persist across re-scores — if a flagged score changes, the change is shown alongside the flag, never applied silently — and stay open until someone explicitly resolves them with a recorded name, note, and date.
...
**When the AI is wrong.** Faculty will sometimes disagree with the AI's reading of their own course. The framework's commitment is that disagreements persist (as flags) rather than vanishing into a button click — and that flagged cells are not silently overwritten in future re-scores. The flag mechanism is live: ⚑ buttons on program-matrix cells and review-panel competencies file a named dispute note; flagged scores are shown alongside their current value if the score changes; flags stay open until someone resolves them with a recorded name, note, and date. A program-wide "⚑ N open" panel surfaces all open flags with drift display. How systematic patterns of open flags inform future prompt or rubric changes is Phase 2 work.
...
**Cost.** AI-model cost is a manageable variable as adoption grows. The current trial runs against the OpenAI API on credits; if usage scales beyond what credits absorb, the architecture supports a swap to Clemson RCD's campus-hosted models or a local runtime without changes to the application layer. The tool optimizes for prompt-cache reuse and exposes function-tier settings (`/settings`) so the program can tune cost-per-call against analytical depth on a per-function basis.
