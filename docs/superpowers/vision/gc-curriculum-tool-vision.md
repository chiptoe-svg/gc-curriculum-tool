...
**The design intent.** The structural target of Act 2 is the empirical "aha" moment students consistently report from GC 4400 and GC 4060 — the mini-capstones where "it all came together" and they first recognized where they could add professional value. The 3-Act structure positions this integrative threshold at mid-Junior year so the back half of the program — 1.5+ years — is available for career-path-specific Act 3 specialization. This inverts the cornerstone-capstone pattern common in engineering education (where integration is reserved for the senior capstone, leaving no time for post-integration specialization). The framework's bet is that students develop better career-path-specific capability when they reach integration earlier and have substantial time afterward to apply it in a focused way — and that the curriculum analysis is most useful when it diagnoses scaffolding against the act-boundary the program is actually trying to hit.

**Grounding.** The 3-Act structure is a working organizing model adopted by the program coordinator and faculty leadership; it is consistent with established curriculum-design traditions (Bruner's spiral curriculum, the engineering cornerstone-capstone model, the modern medical-education three-phase structure, cognitive apprenticeship) but is not derived from any single one. The scaffolding analysis treats act boundaries as analytical waypoints; if the structure is refined or replaced, the scaffolding-diagnosis logic is revised accordingly. The full research synthesis — what the structure is consistent with, what it isn't grounded in, how it engages the strongest critiques (Lave & Wenger on situated learning, Schön on artificial boundaries, prerequisite-transfer findings), and an honest accounting of which design choices are research-supported vs. stipulated — is in the companion document [**The 3-Act Program Structure: A Research Synthesis**](../../three-act-deep-dive.html).
