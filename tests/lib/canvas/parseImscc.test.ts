...
describe('parseImscc', () => {
  it('parses the fixture cartridge into CanvasCourseData + filtered files', async () => {
    const { data, files, skipped } = await parseImscc('tests/fixtures/sample.imscc');
    ...
    // The image is filtered out of files[] and recorded as an unsupported skip.
    const png = skipped.find(s => s.name.endsWith('.png'));
    expect(png?.reason).toBe('unsupported');
  });
