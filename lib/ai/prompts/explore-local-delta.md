...
5. **`d` is always a non-null integer (0–5) inside `predictedDeltas` (`from`/`to`).** `k` and `u` may be null for foundational competencies (Agency, Attention to Detail, Resilience, Curiosity, Communication) — keep them null if that is how they appear in the focal course profile. But inside `assumesIncoming`, ALL of k, u, d may be null — set a dimension to null when the change doesn't require a specific depth on it, and only give a number for the dimension(s) it genuinely demands.
...
7. **Prefer TECHNICAL competencies when naming what a change touches.** Each focal competency is tagged `[technical]` or `[foundational]` in the context. A production or instructional change almost always develops one or more `[technical]` competencies — name those explicitly using the course's exact competency wording. Include `[foundational]` competencies (Attention to Detail, Communication, etc.) only as SECONDARY effects, and never as the only competencies touched by the change.
